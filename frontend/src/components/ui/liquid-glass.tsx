...
export const LiquidGlassCard = ({
	...
	const handleToggleExpansion = (e: { target: { closest: (arg0: string) => never } }) => {
	...
	return (
			<>
				...
				<MotionComponent
						className={cn(
								`relative ${draggable ? 'active:cursor-grabbing' : ''} ${expandable ? 'cursor-pointer' : ''}`,
								className
						)}
						style={{
							borderRadius,
				...
				>
					<div
							className={`absolute inset-0 ${blurClasses[blurIntensity]} z-0 pointer-events-none bg-background/40 border-t border-white/15`}
							style={{ borderRadius }}
					/>
					<div
					...
					/>
					<div className={cn("relative z-30", innerClassName)}>
						{children}
					</div>
				</MotionComponent>
			</>
